...
function solve_for_pressure!(pressure, solver, arch, grid, U★, Δt)
    ...
    @launch(device(arch), config=launch_config(grid, :xyz),
            calculate_pressure_right_hand_side!(RHS, solver.type, arch, grid, U★, Δt))
...
"""
function calculate_pressure_right_hand_side!(RHS, solver_type, arch, grid, U★, Δt)
